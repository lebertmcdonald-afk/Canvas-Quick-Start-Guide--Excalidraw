...
- **Day 18 (done):** the full four-hint chain is real: `shape-tool` → `labeling` ("Double-click a shape to name this step") → `connecting` ("Draw an arrow to connect two shapes") → `save` ("Your drawing auto-saves in this browser. Use the menu to save a copy."). Each auto-activates once the previous one completes. `labeling` completes on a genuine *bound* label (`containerId` set), not any text on the canvas; `connecting` completes on an arrow bound at *both* ends to two *different* shapes, not a stray or self-looped one. All verified by hand in-browser, not just typechecked -- including tracking down a false alarm where double-clicking an unfilled shape didn't actually bind the text; selecting the shape and pressing Enter did (not a code bug, a quirk of that specific interaction).
  ...
  - **Hint completion (PRD P0):** per-hint and mark-aware. `shape-tool` wants any authored mark (not images/imports, per PRD §3); `labeling` wants specifically a bound text label; `connecting` wants an arrow bound at both ends to two different shapes.

  Completion is tracked by **satisfaction, not id novelty** -- `notifySceneChange` diffs *which element ids currently satisfy the active hint's predicate* against the previous check, rather than "have I seen this id before." That distinction is load-bearing, found via a real in-browser bug: Excalidraw assigns an arrow its id before a drag resolves which shape it binds to, so `onChange` can fire while the arrow exists, unbound, with an id that then gets baselined as "already known." The old id-novelty check permanently missed that same arrow becoming bound moments later -- the id was never new, only its bindings changed. Labeling has the identical exposure (a bound-text element could in principle appear before its `containerId` resolves), so the fix applies to both, not just connecting. Covered by a dedicated regression test (`tests/quickstart.test.tsx`, "an arrow that starts unbound and becomes bound with the SAME id completes connecting").
