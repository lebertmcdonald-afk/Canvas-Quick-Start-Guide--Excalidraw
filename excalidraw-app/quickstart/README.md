...
- **Remote collaborator edits (done, Mofazzal):** `Collab.handleRemoteSceneUpdate` wraps its `updateScene` call in `remoteScene.ts`'s depth-counted flag, so `App.tsx`'s `onChange` can tell a collaborator's write from the local user's. Remote elements are baselined: they can't complete a hint and can't dismiss the opt-in prompt (P1 is the local user starting to draw on their own). A later local mark still completes the active hint. Covered by unit tests; still unverified against a live two-tab collaborative session.
