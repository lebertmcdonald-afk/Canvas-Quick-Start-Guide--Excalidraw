...
describe("explicit-save tracking (unsaved = user hasn't clicked save)", () => {
  ...
  it("the same explicit-work signal drives the close-time alert", () => {
    // the browser's own beforeunload confirm fires for the full
    // hasUnsavedWork set, not just technical data loss: drawn-but-never-
    // saved work must alert on close even though autosave has it
    noteSceneChange([makeElement("a")]);
    expect(
      hasUnsavedWork([makeElement("a")], { fileStorage: makeFileManager() }),
    ).toBe(true);
